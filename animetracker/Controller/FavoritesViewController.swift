...
class FavoritesViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        TabBarViewController.initializeTableView(vc: self, tableView: tableView, cellId: Constants.cellId, title: Constants.favorites, imageName: Constants.heart)
        reloadTableView()
        observeUserAnimes()
    }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if let animeId = anime.id {
             Database.database().reference().child(Constants.userFavorites).child(uid).child(animeId).removeValue { (error, ref) in
    ...
    func observeUserAnimes() {
        let uid = Auth.auth().currentUser!.uid
        let ref = Database.database().reference().child(Constants.userFavorites).child(uid)
